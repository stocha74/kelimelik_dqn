...
class KelimelikDQNEnv(gym.Env):
    def __init__(self, sozluk, tahta_puanlari2, harf_stogu, debug=False, log_dir="logs", auto_save_excel=True, maintenance_every_n_turns=8, max_dawg_cache_size=2):
        super().__init__()
        self.sozluk = sozluk
        ...
        self.maintenance_every_n_turns = max(1, int(maintenance_every_n_turns))
        self.max_dawg_cache_size = max(1, int(max_dawg_cache_size))
    ...
    def _periodic_cleanup(self):
        """
        Uzun eğitimlerde hız düşüşünü azaltmak için aralıklı bakım:
        - DAWG cache boyutunu sınırlar (gerekirse mevcut sözlük cache'ini tutar)
        - Python garbage collector tetikler
        """
        cache = getattr(ke.hamle_cok_kriterli, "_dawg_cache", None)
        if isinstance(cache, dict) and len(cache) > self.max_dawg_cache_size:
            current_key = (id(self.sozluk), len(self.sozluk))
            keep = {}
            if current_key in cache:
                keep[current_key] = cache[current_key]
            cache.clear()
            cache.update(keep)
    ...
    def step_emektar(self, action_id, action_source="policy"):
        ...
        self.turn += 1
        if self.turn % self.maintenance_every_n_turns == 0:
